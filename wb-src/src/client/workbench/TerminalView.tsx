...
export function TerminalView({
  ...
  // ★ 2026-09-08 终端断连自动重连：EventSource fatal close（readyState=CLOSED）
  //   后不再直接报错，而是退避重试若干次；连续失败才提示用户手动重连。
  //   根因：nginx 曾缓冲 SSE ping 帧 → 浏览器误判死链 → CLOSED，旧逻辑直接死。
  ...
  useEffect(() => {
    ...
    const connect = (): void => {
      ...
      source.onerror = () => {
        ...
        // 退避重连：1s, 2s, 4s, 8s, 16s，最多 MAX_RECONNECT 次；超过才报错。
        retryCountRef.current += 1
        if (retryCountRef.current <= MAX_RECONNECT) {
          const delay = Math.min(1000 * 2 ** (retryCountRef.current - 1), 16000)
          setStatus('idle')
          setError(null)
