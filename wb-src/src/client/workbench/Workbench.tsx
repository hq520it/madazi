...
import { subscribeChatOpen } from './chat-open.ts'
import { collectChangedPaths, subscribeOpenDiffReview } from './change-review.ts'
import { injectSplashStyle, onGitIdeMounted } from './splash.ts'

// fork 定制：bundle 加载即注入启动隐藏 CSS（幂等；早于任何挂载流程）
injectSplashStyle()
import {
...
// ★ AI 自动打开浏览器时的兜底地址：最近打开过的项目预览（localStorage 历史首条）
function lastPreviewUrl(): string {
  try {
    const h = JSON.parse(localStorage.getItem('madazi.previewHistory') || '[]')
    if (Array.isArray(h) && h.length > 0 && h[0] && typeof h[0].url === 'string') return h[0].url
  } catch { /* ignore */ }
  return ''
}
...
// 从工作区路径提取项目 id（/app/generated/<uuid>）
function projectIdFromPath(p: string | undefined): string {
  if (!p) return ''
  const m = p.match(/\/generated\/([0-9a-fA-F-]{36})/)
  return m ? m[1] : ''
}

// 默认预览地址：当前项目（会话工作区）→ 最近历史 → 项目列表首个（异步）
function defaultPreviewUrl(projectId?: string): Promise<string> {
  if (projectId) {
    const u = pvUrlFor(projectId)
    if (u !== '') return Promise.resolve(u)
  }
  const last = lastPreviewUrl()
  if (last !== '') return Promise.resolve(last)
  // ★ 网络抖动韧性：/api/projects 失败重试（否则回填预览失败 → 浏览器 iframe 空 →
  //   AI 浏览器指令报「浏览器未打开或无页面」）
  const attempt = (n: number): Promise<string> => {
    return fetch('/api/projects', { credentials: 'same-origin' })
      .then(r => r.json())
      .then((j: unknown) => {
        if (!Array.isArray(j)) return ''
        const list = j as Array<{ id?: string; status?: string }>
        const p = list.find(x => x.status !== 'archived') || list[0]
        return p && p.id ? pvUrlFor(String(p.id)) : ''
      })
      .catch(() => {
        if (n > 0) return new Promise((res) => setTimeout(() => res(attempt(n - 1)), 1000))
        return ''
      })
  }
  return attempt(2)
...
function pvSvcGlobal(): { [k: string]: any } | null {
  return (window as unknown as { __madaziSvc?: { [k: string]: any } }).__madaziSvc || null
}

// ★ 确保预览已运行后再载入 pv URL（镜像手动选项目的流程：未运行 → 启动 → 轮询 running → 导航），
//   避免加载未就绪的预览报「连接拒绝」，也省得用户手动刷新
function ensurePreviewRunning(projectId: string, url: string, tabId: string): void {
  const svc = pvSvcGlobal()
  if (!svc) { commitBrowserUrl(tabId, url); return }
  const isRunning = (d: any): boolean => {
    const v = d && d.ok ? d.value : d
    return !!v && (v.running === true || v.status === 'running')
  }
  svc.previewStatus(projectId).then((d: any) => {
    if (isRunning(d)) { commitBrowserUrl(tabId, url); return }
    try { svc.previewStart(projectId) } catch { /* ignore */ }
    let tries = 0
    const poll = (): void => {
      tries += 1
      if (tries > 60) { commitBrowserUrl(tabId, url); return } // 3min 超时兜底
      svc.previewStatus(projectId).then((d2: any) => {
        if (isRunning(d2)) { commitBrowserUrl(tabId, url); return }
        window.setTimeout(poll, 3000)
      }).catch(() => { window.setTimeout(poll, 3000) })
    }
    poll()
  }).catch(() => { commitBrowserUrl(tabId, url) })
}
...
function WorkbenchInner(props: WorkbenchProps) {
  ...
  // ★ AI 自动打开内置浏览器：madazi 插件 browser_* 指令发现浏览器未打开时调用。
  //   ★ fork 定制（2026-09-04 每会话独立浏览器 tab）：按会话建/找 browser tab——
  //     会话 B 后台生成时用 {activate:false} 建隐藏 tab（不抢当前视图，iframe 由
  //     EditorPane 隐藏常驻堆保持挂载）；当前会话默认 activate 并切到可见。
  const workspaceRef = useRef<{ path?: string } | undefined>(undefined)
  useEffect(() => {
    const ensure = (sid: string | undefined, url?: string, opts?: { activate?: boolean }): string => {
      const act = opts?.activate !== false
      const tid = sid ? browserTabOfSession(sid) : null
      let tab = tid ? tabsRef.current.find(t => t.id === tid) ?? null : null
      if (tab === null || tab.kind !== 'browser') {
        tab = nextBrowserTab(tabsRef.current)
        ensureBrowserTab(tab.id)
        if (sid) bindBrowserTabToSession(sid, tab.id)
        setTabs(current => [...current, tab as FileTab])
      }
      // ★ 当前会话（activate）才切到浏览器视图（否则 BrowserView 不挂载可见 iframe）；
      //   后台会话保持隐藏，仅由隐藏常驻堆挂载 iframe 供泵操作
      if (act) {
        setActiveBrowserId(tab.id)
        setActiveId(tab.id)
        patchWorkbenchChrome({ editorOpen: true })
      }
      if (url !== undefined && url !== '') { commitBrowserUrl(tab.id, url); return tab.id }
      if (readBrowserTab(tab.id).committed !== '') return tab.id
      // 空浏览器 → 解析目标预览 URL，先确保预览已运行再载入（避免加载未就绪预览报错）。
      // 仅对当前会话可靠（默认预览来自当前工作区路径）；后台会话由插件显式传 URL。
      const projectId = projectIdFromPath(workspaceRef.current?.path)
      void defaultPreviewUrl(projectId).then(u => {
        if (u === '' || readBrowserTab(tab!.id).committed !== '') return
        if (projectId === '') { commitBrowserUrl(tab!.id, u); return }
        ensurePreviewRunning(projectId, u, tab!.id)
      })
      return tab.id
    }
    const drop = (sid: string): void => {
      const tid = browserTabOfSession(sid)
      if (tid === null) return
      unbindBrowserSession(sid)
      const t = tabsRef.current.find(t => t.id === tid)
      if (t && t.kind === 'browser') closeTabsRef.current?.([tid])
    }
    const w = window as unknown as {
      __madaziBrowserEnsure?: (sid?: string, url?: string, opts?: { activate?: boolean }) => string
      __madaziBrowserDrop?: (sid: string) => void
      __madaziBrowserBySession?: Map<string, string>
    }
    w.__madaziBrowserEnsure = ensure
    w.__madaziBrowserDrop = drop
    // ★ 实时镜像会话↔tab 映射（插件泵按 data-browser-tab 定位专属 iframe）。
    //   用 getter 惰性返回快照，插件每次读取即最新。
    let mirror: Map<string, string> = new Map()
    Object.defineProperty(w, '__madaziBrowserBySession', {
      configurable: true,
      get: () => { mirror = snapshotSessionTabs(); return mirror },
    })
    return () => {
      if (w.__madaziBrowserEnsure === ensure) delete w.__madaziBrowserEnsure
      if (w.__madaziBrowserDrop === drop) delete w.__madaziBrowserDrop
    }
  }, [])
  ...
  // AI 改动审查：当前会话全窗口出现过的变更路径集合。
  // 会话 cwd 即工作区根，model-facing 相对路径可直接与聊天点击映射出的
  // workspace relPath 比对（详见 chat-open.ts / change-review.ts）。
  const sessionNodes = props.useSession?.(state => state.nodes) as readonly unknown[] | undefined
  const sessionTurnEnds = props.useSession?.(state => state.turnEnds) as ReadonlyMap<number, number> | undefined
  const changedPaths = useMemo(
    () => collectChangedPaths({ nodes: sessionNodes, turnEnds: sessionTurnEnds }),
    [sessionNodes, sessionTurnEnds],
  )
  ...
  useLayoutEffect(() => {
    ...
    if (!split) {
      ...
      // fork 定制：用户关闭 workbench → 豁免启动隐藏，恢复默认聊天界面可见
      onGitIdeMounted(false)
      return
    }
    scroll.dataset.gitIde = ''
    onGitIdeMounted(true)
  ...
  const openFile = useCallback(async (path: string): Promise<void> => {
    ...
    setBuffers(current => current[path] !== undefined ? current : ({
      ...current,
      [path]: { path, original: result.value.content, draft: result.value.content, language: result.value.language, version: result.value.version },
    }))
  }, [client, workspaceId])

  /** Re-read a file from disk and replace its buffer (conflict dialog "reload"). */
  const reloadFile = useCallback(async (path: string): Promise<void> => {
    if (workspaceId === undefined) return
    const result = await client.readFile(workspaceId, path)
    if (!result.ok) return
    setBuffers(current => current[path] === undefined ? current : ({
  ...
  // fork 定制：聊天侧文件点击（总结区产物 chip / 工具行文件 / 结束语 mention）
  // 改道本工作台。包装点见 chat-open.ts（index.ts 安装）；工作台关闭时点击
  // 会重新启用并展开编辑器列。本会话 AI 改过的文件改开 diff 审查视图
  // （可按块/按文件撤销），其余开普通编辑器。
  useEffect(() => subscribeChatOpen(request => {
    if (request.workspaceId !== workspaceId) return
    if (request.relPath === '') {
      patchWorkbenchChrome({ enabled: true, sideOpen: true, sideTab: 'files' })
      return
    }
    patchWorkbenchChrome({ enabled: true })
    if (changedPaths.has(request.relPath)) {
      openDiff(request.relPath, false)
      return
    }
    void openFile(request.relPath)
  }), [workspaceId, openFile, changedPaths, openDiff])

  // AI 改动审查条（conversation.input.dock）请求打开某文件的 diff 审查视图。
  useEffect(() => subscribeOpenDiffReview(request => {
    if (workspaceId === undefined) return
    patchWorkbenchChrome({ enabled: true })
    openDiff(request.path, false)
  }), [workspaceId, openDiff])
  ...
  // ★ fork 定制：ensure 的 drop(sid) 需要调 closeTabs 关 tab；用 ref 绑定避免 useEffect 依赖
  const closeTabsRef = useRef<((ids: string[]) => void) | null>(null)

  const closeTabs = (ids: string[]): void => {
    closeTabsRef.current = closeTabs
    const closable = ids.filter(id => id !== TERMINAL_TAB_ID)
    if (closable.length === 0) return
    // ★ fork 定制：关闭 browser tab 时反查会话并解绑（该会话下次操作按需重建 tab）
    for (const id of closable) {
      if (id.startsWith('browser:')) {
        const sid = sessionOfBrowserTab(id)
        if (sid !== null) unbindBrowserSession(sid)
      }
    }
    ...
    <>
      ...
      {editorOpen ? (
        <EditorPane
          ...
          onSaved={(path, content, version) => {
            setBuffers(current => current[path] === undefined
              ? current
              : { ...current, [path]: { ...current[path]!, original: content, draft: content, version } })
          }}
          onReload={(path) => { void reloadFile(path) }}
