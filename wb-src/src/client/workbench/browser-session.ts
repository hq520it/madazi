...
// ★ fork 定制：会话 ↔ 浏览器 tab 绑定（2026-09-04 每会话独立浏览器 tab）。
//   Workbench.__madaziBrowserEnsure 按会话建/找 tab 时写入；插件泵用
//   window.__madaziBrowserBySession 定位该会话专属 iframe（data-browser-tab）。
//   用户手关 tab / 会话删除时解绑。同一会话复用同一个 tab，不随会话切换重绑。
const sessionTabs = new Map<string, string>()

export function bindBrowserTabToSession(sid: string, tabId: string): void {
  if (!sid || !tabId) return
  sessionTabs.set(sid, tabId)
}

export function unbindBrowserSession(sid: string): void {
  sessionTabs.delete(sid)
}

export function browserTabOfSession(sid: string): string | null {
  return sessionTabs.get(sid) ?? null
}

export function sessionOfBrowserTab(tabId: string): string | null {
  for (const [sid, tid] of sessionTabs) if (tid === tabId) return sid
  return null
}

export function snapshotSessionTabs(): Map<string, string> {
  return new Map(sessionTabs)
}
