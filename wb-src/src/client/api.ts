...
export interface GitClient {
  ...
  /**
   * Save a text file. `expectedVersion` = the `FsFileSnapshot.version` captured
   * at read time: when provided, a concurrent change (AI session / another
   * member) rejects with `FS_STALE_VERSION` instead of silently overwriting.
   * Omit it to force-write (user confirmed "overwrite" in the conflict dialog).
   */
  writeFile(workspaceId: string, path: string, content: string, expectedVersion?: string): Promise<GitResult<FsWriteResult>>
...
export function createGitClient(): GitClient {
  return {
    ...
    writeFile: (workspaceId, path, content, expectedVersion) => request('/git/fs/write', {
      method: 'POST', body: JSON.stringify({ workspaceId, path, content, expectedVersion }),
    }),
