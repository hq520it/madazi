...
export interface FsFileSnapshot {
  ...
  ignored: boolean
  /**
   * Optimistic-lock version captured at read time (`dev:ino:size:mtimeNs:ctimeNs`,
   * same format as dsh-fs-local). Send it back as `expectedVersion` on write to
   * detect concurrent edits (AI session / another user) instead of blind overwrite.
   */
  version?: string
}
...
export interface FsWriteResult {
  path: string
  size: number
  /** New version after the write; feed it into the next save's `expectedVersion`. */
  version?: string
}
