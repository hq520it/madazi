...
function versionOfStat(info: Stats): string {
  const ns = (value: bigint | number | undefined): string => (typeof value === 'bigint' ? value.toString() : String(value ?? 0))
...
async function writeAtomic(abs: string, content: string): Promise<Stats> {
...
export class WorkspaceFs {
  ...
  async read(root: string, filePath: string): Promise<FsFileSnapshot> {
    ...
    return { path: rel, content: buffer.toString('utf8'), size: buffer.length, language: languageOf(rel), ignored, version: versionOfStat(info) }
  }
   ...
   */
  async write(root: string, filePath: string, content: string, expectedVersion?: string): Promise<FsWriteResult> {
    const rel = assertSafeWorkspacePath(root, filePath)
    if (rel === '') throw new GitError('FS_IS_DIRECTORY')
    ...
    return withWriteLock(abs, async () => {
      try {
        ...
        if (info.isDirectory()) throw new GitError('FS_IS_DIRECTORY')
        if (expectedVersion !== undefined && versionOfStat(info) !== expectedVersion) {
          throw new GitError('FS_STALE_VERSION')
        }
      } catch (error) {
        if (error instanceof GitError) throw error
        if (!isNotFound(error)) throw new GitError('FS_WRITE_FAILED')
        if (expectedVersion !== undefined) {
          // The file vanished after it was read — treat as conflict, not create.
          throw new GitError('FS_STALE_VERSION')
        }
      ...
      return { path: rel, size: Buffer.byteLength(content, 'utf8'), version: versionOfStat(info) }
    })
  }
}