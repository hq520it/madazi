...
function agentFor(ctx: Context, sessionId: string | undefined): unknown {
  ...
  // ★ madazi fork：agents.get 对不存在/已销毁的会话 id 会 throw（binding 同理），
  //   刷新后会话尚未恢复或已被删除时 UsagePanel 会带旧 id 打此端点 → 炸穿 400
  try {
    const agents = ctx.get('agents') as { get?: (id: string) => unknown } | undefined
    const found = agents?.get?.(sessionId)
    if (found !== undefined) return found
  } catch { /* agents 服务不可用 → 走 sessions.binding */ }
  // ★ ctx.get("sessions") 若未 inject 会 throw（cordis 守卫 "without inject"），
  //   且 errors.ts 兜底正则 /without inject|llm/i 会把它吞成 LLM_UNAVAILABLE → 400
  let sessions: Context['sessions'] | undefined
  try {
    sessions = (ctx.sessions ?? ctx.get('sessions')) as Context['sessions'] | undefined
  } catch {
    return undefined
  }
  try {
...
function resolveRoute(ctx: Context, sessionId: string | undefined): Route | undefined {
...
export async function readProviderUsage(
  ...
  // ★ madazi fork：无可用路由（会话 agent 未就绪/已销毁且无默认模型）时降级为
  //   unsupported 快照而非 throw——刷新后 agent 恢复前 UsagePanel 会带着旧
  //   sessionId 打此端点，throw 会让 /git/usage 全量 400 刷控制台
  if (route === undefined) {
    return {
      provider: '',
      providerName: '',
