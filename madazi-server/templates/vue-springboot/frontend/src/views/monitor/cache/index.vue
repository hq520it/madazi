            ...
            <table cellspacing="0" style="width: 100%">
              <tbody>
                ...
                <tr>
                  ...
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="cache.info">{{ cache.info.used_memomadazi_human }}</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell">使用CPU</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="cache.info">{{ parseFloat(cache.info.used_cpu_user_children).toFixed(2) }}</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell">内存配置</div></td>
                  <td class="el-table__cell is-leaf"><div class="cell" v-if="cache.info">{{ cache.info.maxmemomadazi_human }}</div></td>
                </tr>
                <tr>
...
function getList() {
  ...
  getCache().then(response => {
    ...
    usedmemoryInstance.setOption({
      tooltip: {
        formatter: "{b} <br/>{a} : " + cache.value.info.used_memomadazi_human
      },
      series: [
        {
          name: "峰值",
          ...
          max: 1000,
          detail: {
            formatter: cache.value.info.used_memomadazi_human
          },
          data: [
            {
              value: parseFloat(cache.value.info.used_memomadazi_human),
              name: "内存消耗"
            }
          ]
        }
