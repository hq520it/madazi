window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		let hiddenSessionIds = /* @__PURE__ */ new Set();
		...
		*/
		function subscribeMembership(listener) {
		...
		*/
		async function refreshMembership() {
			if (authFailed && globalThis.__madaziLoggedIn === true) {
				authFailed = false;
				if (timer === void 0) scheduleTick(0);
			}
			...
			try {
				...
				if (!changed) {
					for (const id of allowed) if (!table.allowed.has(id)) {
						changed = true;
						break;
					}
				}
		...
		*/
		function startMembershipWatch() {
			...
			watchStarted = true;
			globalThis.__madaziMembershipRefresh = refreshMembership;
			scheduleTick(0);
		}
		/**
		...
		*/
		function gatedWorkspaces(workspaces) {
			if (!Array.isArray(workspaces)) return workspaces;
			if (workspaceCache !== void 0 && workspaceCache.raw === workspaces && workspaceCache.v === version) return workspaceCache.out;
			hiddenSessionIds = /* @__PURE__ */ new Set();
			const kept = [];
		...
		*/
		function gatedList(list) {
			if (list === null || typeof list !== "object") return list;
			if (listCache !== void 0 && listCache.raw === list && listCache.v === version) return listCache.out;
			const deniedRow = (sid) => {
				if (hiddenSessionIds.has(sid)) return true;
				const summary = list.byId[sid];
				if (summary !== void 0 && isDeniedPath(summary.cwd)) return true;
				return false;
			};
			...
			for (const key in list.byId) {
				const sid = key;
				if (Object.hasOwn(list.byId, key) && !deniedRow(sid)) byId[sid] = list.byId[sid];
			}
			const current = list.current !== void 0 && !deniedRow(list.current) ? list.current : void 0;
			const subagentsByParent = {};
			for (const parentKey in list.subagentsByParent) {
				const parent = parentKey;
				if (Object.hasOwn(list.subagentsByParent, parentKey) && !hiddenSessionIds.has(parent)) subagentsByParent[parent] = list.subagentsByParent[parent];
			}
			const jobsBySession = {};
			for (const jobKey in list.jobsBySession) {
				const sid = jobKey;
				if (Object.hasOwn(list.jobsBySession, jobKey) && !hiddenSessionIds.has(sid)) jobsBySession[sid] = list.jobsBySession[sid];
			}
			const out = {
				ids,
				byId,
		...
		var UiWorkspaceService = class extends _deepseek_ai_cordis.Service {
			...
			*/
			...
			async connectWorkspace(workspaceId) {
				...
				if (isDeniedPath(workspace.path)) throw new Error("uiWorkspace.connectWorkspace: workspace denied");
			...
			startSession(workspaceId) {
				...
				const currentWorkspaceId = current === void 0 ? void 0 : gateItems.find((item) => item.sessionIds.includes(current))?.workspaceId;
				const recent = workspace.phase === "ready" && sessions.phase === "ready" ? recentWorkspace(gateItems, sessions.byId) : void 0;
			...
			watchNavigation() {
				...
				const reconcile = () => {
					...
					if (sessions.current !== void 0) {
						...
						if (restored !== void 0 && isDeniedPath(restored.cwd)) {
							this.sessions.clear();
							return;
						}
						initial = "done";
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:./dsh-src/packages/client/ui-workspace/src/client/rows/Rows.module.css.mjs
		const css$2 = ".b5-Cbq_projectRow,.b5-Cbq_sessionRow{cursor:pointer;user-select:none;color:var(--dsw-alias-label-primary);border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}.b5-Cbq_projectRow:hover,.b5-Cbq_sessionRow:hover,.b5-Cbq_sessionRow.b5-Cbq_selected{background:var(--dsw-alias-interactive-bg-hover)}.b5-Cbq_searchResultRow{box-sizing:border-box;cursor:pointer;text-align:left;width:100%;min-height:48px;color:var(--dsw-alias-label-primary);background:0 0;border:none;border-radius:8px;flex-direction:column;align-items:stretch;padding:4px 8px;display:flex}.b5-Cbq_searchResultRow:hover,.b5-Cbq_searchResultRow.b5-Cbq_selected{background:var(--dsw-alias-interactive-bg-hover)}.b5-Cbq_searchResultHeading{align-items:center;min-width:0;display:flex}.b5-Cbq_searchResultTitle{text-overflow:ellipsis;white-space:nowrap;flex:0 auto;min-width:0;margin-left:4px;font-size:14px;line-height:20px;overflow:hidden}.b5-Cbq_searchResultMeta{align-items:center;gap:6px;min-width:0;margin-left:20px;display:flex}.b5-Cbq_searchResultWorkspace,.b5-Cbq_searchResultSnippet{text-overflow:ellipsis;white-space:nowrap;font-size:12px;line-height:17px;overflow:hidden}.b5-Cbq_searchResultWorkspace{max-width:40%;color:var(--dsw-alias-label-tertiary);flex:none}.b5-Cbq_searchResultSnippet{min-width:0;color:var(--dsw-alias-label-secondary);flex:1}.b5-Cbq_projectRow{box-sizing:border-box;align-items:center;height:34px}.b5-Cbq_projectRow .b5-Cbq_rowActions{height:20px}.b5-Cbq_sessionRow{height:32px;animation:b5-Cbq_row-in .15s var(--ds-ease-in-out);gap:0}.b5-Cbq_sessionRow .b5-Cbq_title{margin:0 6px 0 4px}.b5-Cbq_flatSessionRowWithoutStatus .b5-Cbq_title{margin-left:0}@keyframes b5-Cbq_row-in{0%{opacity:0}}.b5-Cbq_slot{width:16px;height:20px;color:var(--dsw-alias-label-tertiary);flex:none;justify-content:center;align-items:center;display:inline-flex}.b5-Cbq_visuallyHidden{clip:rect(0 0 0 0);white-space:nowrap;width:1px;height:1px;position:absolute;overflow:hidden}.b5-Cbq_folderActive{color:var(--dsw-alias-state-business-primary)}.b5-Cbq_projectRow .b5-Cbq_chevron{display:none}.b5-Cbq_projectRow:hover .b5-Cbq_chevron{display:inline-flex}.b5-Cbq_projectRow:hover .b5-Cbq_folder{display:none}.b5-Cbq_arrow{transition:transform .15s var(--ds-ease-in-out)}.b5-Cbq_arrowOpen{transform:rotate(90deg)}.b5-Cbq_projectText{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.b5-Cbq_title{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-size:14px;line-height:20px;overflow:hidden}.b5-Cbq_renameInput{border:.5px solid var(--dsw-alias-border-l4);background:var(--dsw-alias-button-elevated-fill);min-width:0;color:inherit;border-radius:4px;outline:none;padding:0 2px;font-size:14px;line-height:20px}.b5-Cbq_sessionRow .b5-Cbq_title{flex:1}.b5-Cbq_meta{text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:20px;overflow:hidden}.b5-Cbq_time{color:var(--dsw-alias-label-tertiary);flex:none;font-size:12px;line-height:20px}.b5-Cbq_scheduleIndicator{width:16px;height:20px;color:var(--dsw-alias-label-tertiary);flex:none;justify-content:center;align-items:center;margin-right:6px;display:inline-flex}.b5-Cbq_searchScheduleIndicator{margin-left:4px;margin-right:0}.b5-Cbq_dot{flex:none}.b5-Cbq_rowActions{flex:none;align-items:center;gap:12px;display:none}.b5-Cbq_projectRow:hover .b5-Cbq_rowActions,.b5-Cbq_sessionRow:hover .b5-Cbq_rowActions,.b5-Cbq_projectRow.b5-Cbq_menuOpen .b5-Cbq_rowActions,.b5-Cbq_sessionRow.b5-Cbq_menuOpen .b5-Cbq_rowActions{display:inline-flex}.b5-Cbq_sessionRow:hover .b5-Cbq_time,.b5-Cbq_sessionRow.b5-Cbq_menuOpen .b5-Cbq_time{display:none}.b5-Cbq_projectRow.b5-Cbq_menuOpen,.b5-Cbq_sessionRow.b5-Cbq_menuOpen{background:var(--dsw-alias-interactive-bg-hover)}.b5-Cbq_sessionRow.b5-Cbq_dropBefore,.b5-Cbq_sessionRow.b5-Cbq_dropAfter{position:relative}.b5-Cbq_sessionRow.b5-Cbq_dropBefore:before,.b5-Cbq_sessionRow.b5-Cbq_dropAfter:after{content:\"\";z-index:1;background:linear-gradient(55deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 0 / 5px 7px no-repeat, linear-gradient(125deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 5px / 5px 7px no-repeat, linear-gradient(var(--dsw-alias-state-business-primary) 0 0) 4px 5px / calc(100% - 4px) 2px no-repeat;pointer-events:none;height:12px;position:absolute;left:0;right:4px}.b5-Cbq_sessionRow.b5-Cbq_dropBefore:before{top:-7px}.b5-Cbq_sessionRow.b5-Cbq_dropAfter:after{bottom:-7px}.b5-Cbq_hoverContent{flex-direction:column;gap:8px;display:flex}.b5-Cbq_hoverTitle{color:#fff;overflow-wrap:break-word;font-size:14px;line-height:20px}.b5-Cbq_hoverPath{color:#cfd3d6;word-break:break-all;font-size:12px;line-height:16px}.b5-Cbq_hoverTime{color:#cfd3d6;font-size:12px;line-height:16px}.b5-Cbq_hoverStatus{color:#adb2b8;align-items:center;gap:8px;font-size:12px;line-height:20px;display:flex}.b5-Cbq_iconButton{cursor:pointer;width:16px;height:16px;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.b5-Cbq_iconButton:hover{color:var(--dsw-alias-label-primary)}.b5-Cbq_chevron{color:var(--dsw-alias-label-caption)}@media (prefers-reduced-motion:reduce){.b5-Cbq_sessionRow,.b5-Cbq_arrow{transition:none;animation:none}}";
		...
		var Rows_module_css_default = {
			"arrow": "b5-Cbq_arrow",
			"arrowOpen": "b5-Cbq_arrowOpen",
			"chevron": "b5-Cbq_chevron",
			"dot": "b5-Cbq_dot",
			"dropAfter": "b5-Cbq_dropAfter",
			"dropBefore": "b5-Cbq_dropBefore",
			"flatSessionRowWithoutStatus": "b5-Cbq_flatSessionRowWithoutStatus",
			"folder": "b5-Cbq_folder",
			"folderActive": "b5-Cbq_folderActive",
			"hoverContent": "b5-Cbq_hoverContent",
			"hoverPath": "b5-Cbq_hoverPath",
			"hoverStatus": "b5-Cbq_hoverStatus",
			"hoverTime": "b5-Cbq_hoverTime",
			"hoverTitle": "b5-Cbq_hoverTitle",
			"iconButton": "b5-Cbq_iconButton",
			"menuOpen": "b5-Cbq_menuOpen",
			"meta": "b5-Cbq_meta",
			"projectRow": "b5-Cbq_projectRow",
			"projectText": "b5-Cbq_projectText",
			"renameInput": "b5-Cbq_renameInput",
			"row-in": "b5-Cbq_row-in",
			"rowActions": "b5-Cbq_rowActions",
			"scheduleIndicator": "b5-Cbq_scheduleIndicator",
			"searchResultHeading": "b5-Cbq_searchResultHeading",
			"searchResultMeta": "b5-Cbq_searchResultMeta",
			"searchResultRow": "b5-Cbq_searchResultRow",
			"searchResultSnippet": "b5-Cbq_searchResultSnippet",
			"searchResultTitle": "b5-Cbq_searchResultTitle",
			"searchResultWorkspace": "b5-Cbq_searchResultWorkspace",
			"searchScheduleIndicator": "b5-Cbq_searchScheduleIndicator",
			"selected": "b5-Cbq_selected",
			"sessionRow": "b5-Cbq_sessionRow",
			"slot": "b5-Cbq_slot",
			"time": "b5-Cbq_time",
			"title": "b5-Cbq_title",
			"visuallyHidden": "b5-Cbq_visuallyHidden"
		};
		//#endregion
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:./dsh-src/packages/client/ui-workspace/src/client/WorkspacePicker.module.css.mjs
		const css$1 = "._9w1FvG_modalAction{min-width:72px}._9w1FvG_modalError,._9w1FvG_menuStatus{margin-top:8px;font-size:12px;line-height:18px}._9w1FvG_modalError{color:var(--dsw-alias-state-error-primary)}._9w1FvG_menuStatus{color:var(--dsw-alias-label-secondary)}";
		...
		var WorkspacePicker_module_css_default = {
			"menuStatus": "_9w1FvG_menuStatus",
			"modalAction": "_9w1FvG_modalAction",
			"modalError": "_9w1FvG_modalError"
		};
		//#endregion
		...
		*/
		function WorkspacePickFlow({ t, open, anchorRef, useWorkspaces, createWorkspace, useDirectoryFlow, renderDirectoryFlow, onPick, onClose, addOnly = false, side = "bottom", selectedId }) {
			...
			const gateVersion = (0, react.useSyncExternalStore)(subscribeMembership, getMembershipVersion);
			(0, react.useEffect)(() => {
				startMembershipWatch();
			}, []);
			const workspaces = (0, react.useMemo)(() => gatedWorkspaces(workspaceSnapshot.items), [workspaceSnapshot.items, gateVersion]);
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:./dsh-src/packages/client/ui-workspace/src/client/rows/WorkspaceBrowser.module.css.mjs
		const css = ".RrBZSG_root{--dsh-session-list-edge-inset:var(--dsh-sidebar-inline-padding);--dsh-session-list-scrollbar-width:8px;--dsh-session-list-scrollbar-offset:2px;box-sizing:border-box;min-height:0;padding-right:var(--dsh-session-list-edge-inset);flex-direction:column;flex:1;display:flex}.RrBZSG_root.RrBZSG_rail{padding-right:0}.RrBZSG_iconButton{corner-shape:round;cursor:pointer;width:28px;height:28px;color:var(--dsw-alias-label-secondary);background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.RrBZSG_iconButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.RrBZSG_sectionHeader{box-sizing:border-box;height:36px;color:var(--dsw-alias-label-tertiary);border-radius:12px;flex:none;justify-content:flex-end;align-items:center;gap:4px;margin-bottom:4px;padding-left:4px;display:flex;overflow:hidden}.RrBZSG_root:not(.RrBZSG_rail) .RrBZSG_sectionHeader{margin-top:2px;margin-right:-4px}.RrBZSG_sectionLabel{white-space:nowrap;opacity:1;visibility:visible;min-width:0;max-width:45%;transition:max-width .18s var(--ds-ease-in-out), margin-right .18s var(--ds-ease-in-out), opacity .12s var(--ds-ease-in-out), transform .18s var(--ds-ease-in-out), visibility 0s linear;flex:none;line-height:20px;overflow:hidden}.RrBZSG_sectionLabelHidden{opacity:0;visibility:hidden;max-width:0;margin-right:-4px;transition-delay:0s,0s,0s,0s,.18s;transform:translate(-4px)}.RrBZSG_searchSlot{box-sizing:border-box;min-width:0;max-width:28px;transition:max-width .18s var(--ds-ease-in-out), padding-left .18s var(--ds-ease-in-out);flex:1;align-items:center;margin-left:auto;padding-left:0;display:flex}.RrBZSG_searchSlotExpanded{max-width:100%;padding-left:0}.RrBZSG_headerActions{opacity:1;visibility:visible;max-width:60px;transition:max-width .18s var(--ds-ease-in-out), opacity .12s var(--ds-ease-in-out), transform .18s var(--ds-ease-in-out), visibility 0s linear;flex:none;align-items:center;gap:4px;display:flex;overflow:hidden}.RrBZSG_headerActionsHidden{opacity:0;visibility:hidden;pointer-events:none;max-width:0;transition-delay:0s,0s,0s,.18s;transform:translate(4px)}.RrBZSG_search{box-sizing:border-box;corner-shape:round;cursor:text;width:100%;height:28px;color:var(--dsw-alias-label-secondary);transition:width .18s var(--ds-ease-in-out), padding .18s var(--ds-ease-in-out), border-color .18s var(--ds-ease-in-out), background-color .18s var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;flex:none;align-items:center;gap:0;margin:0;padding:0;display:flex;overflow:hidden}.RrBZSG_searchExpanded{border:.5px solid var(--dsw-alias-border-l4);width:calc(100% + 4px);height:30px;color:var(--dsw-alias-label-caption);background:0 0;border-radius:10px;margin-inline:-2px;padding:0 4px 0 0}.RrBZSG_searchButton{corner-shape:round;cursor:pointer;width:28px;height:28px;color:inherit;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.RrBZSG_searchExpanded .RrBZSG_searchButton{width:28px;height:30px}.RrBZSG_searchButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.RrBZSG_searchExpanded .RrBZSG_searchButton:hover{background:0 0}.RrBZSG_searchInput{opacity:0;pointer-events:none;width:0;min-width:0;color:var(--dsw-alias-label-primary);transition:opacity .12s var(--ds-ease-in-out);background:0 0;border:none;outline:none;flex:1;font-size:13px;line-height:18px}.RrBZSG_searchExpanded .RrBZSG_searchInput{opacity:1;pointer-events:auto;margin-left:-2px}.RrBZSG_searchInput::placeholder{color:var(--dsw-alias-label-tertiary)}.RrBZSG_clearButton{corner-shape:round;cursor:pointer;width:24px;height:24px;color:var(--dsw-alias-label-secondary);background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.RrBZSG_clearButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.RrBZSG_rail .RrBZSG_sectionHeader{justify-content:flex-start;gap:0;margin-bottom:12px;padding-left:0}.RrBZSG_rail .RrBZSG_headerActions{max-width:none}.RrBZSG_rail .RrBZSG_iconButton{width:36px;height:36px;color:var(--dsw-alias-label-primary)}.RrBZSG_rail .RrBZSG_search{background:0 0;border-color:#0000;gap:0;width:36px;height:36px;margin:0 0 12px;padding:0}.RrBZSG_rail .RrBZSG_searchButton{width:36px;height:36px;color:var(--dsw-alias-label-primary)}.RrBZSG_rail .RrBZSG_searchButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.RrBZSG_listArea{min-height:0;margin-left:-4px;margin-right:calc(-1 * var(--dsh-session-list-edge-inset));flex-direction:column;flex:1;padding-left:4px;display:flex;overflow:visible}.RrBZSG_rail .RrBZSG_listArea{margin-left:0;margin-right:0;padding-left:0}.RrBZSG_treeBody{flex-direction:column;flex:1;min-height:0;display:flex;position:relative}.RrBZSG_fade{left:0;right:var(--dsh-session-list-edge-inset);background:linear-gradient(to bottom, transparent, var(--dsw-specific-sidebar-fill));pointer-events:none;height:24px;position:absolute;bottom:0}.RrBZSG_wide{animation:RrBZSG_wide-in .2s var(--ds-ease-in-out)}@keyframes RrBZSG_wide-in{0%{opacity:0}}.RrBZSG_list{min-height:0;margin-left:-4px;margin-right:var(--dsh-session-list-scrollbar-offset);padding-left:4px;padding-right:calc(var(--dsh-session-list-edge-inset) - var(--dsh-session-list-scrollbar-width) - var(--dsh-session-list-scrollbar-offset));scrollbar-gutter:stable;flex:1;padding-bottom:16px;overflow-y:auto}.RrBZSG_flatList>*+*,.RrBZSG_searchTree>[role=treeitem]+[role=treeitem],.RrBZSG_groupSection>*+*{margin-top:2px}.RrBZSG_searchStatus,.RrBZSG_searchWarning{color:var(--dsw-alias-label-tertiary);padding:10px 12px;font-size:12px;line-height:18px}.RrBZSG_searchWarning{color:var(--dsw-alias-label-secondary)}.RrBZSG_groupSection{position:relative}.RrBZSG_groupSection+.RrBZSG_groupSection{margin-top:4px}.RrBZSG_listTopDropIndicator,.RrBZSG_workspaceDropBefore:before,.RrBZSG_workspaceDropAfter:after{content:\"\";z-index:1;background:linear-gradient(55deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 0 / 5px 7px no-repeat, linear-gradient(125deg, transparent calc(50% - 1px), var(--dsw-alias-state-business-primary) calc(50% - 1px) calc(50% + 1px), transparent calc(50% + 1px)) 0 5px / 5px 7px no-repeat, linear-gradient(var(--dsw-alias-state-business-primary) 0 0) 4px 5px / calc(100% - 4px) 2px no-repeat;pointer-events:none;height:12px;position:absolute;left:0;right:0}.RrBZSG_listTopDropIndicator{top:-8px;left:0;right:var(--dsh-session-list-edge-inset)}.RrBZSG_listTopDropActive>.RrBZSG_workspaceDropBefore:first-child:before{display:none}.RrBZSG_workspaceDropBefore:before{top:-8px}.RrBZSG_workspaceDropAfter:after{bottom:-8px}.RrBZSG_sessionOverflowButton{cursor:pointer;text-align:left;width:100%;height:28px;color:var(--dsw-alias-label-tertiary);background:0 0;border:none;border-radius:8px;padding:0 12px 0 28px;font-size:12px}.RrBZSG_groupSection>.RrBZSG_sessionOverflowButton{margin-top:0}.RrBZSG_sessionOverflowButton:hover{color:var(--dsw-alias-label-secondary);background:0 0}.RrBZSG_empty{color:var(--dsw-alias-label-tertiary);padding:16px 12px;font-size:13px}.RrBZSG_searchEmptyClear{border:.5px solid var(--dsw-alias-border-l4);color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border-radius:16px;margin-top:10px;padding:5px 12px;font-size:12px;display:block}.RrBZSG_searchEmptyClear:hover{border-color:var(--dsw-alias-accent-primary,#4c8ffd);color:var(--dsw-alias-accent-primary,#4c8ffd)}.RrBZSG_renameInput{box-sizing:border-box;border:.5px solid var(--dsw-alias-border-l4);width:100%;height:44px;color:var(--dsw-alias-label-primary);background:0 0;border-radius:22px;outline:none;padding:7px 14px;font-size:14px;font-weight:400;line-height:22px}.RrBZSG_renameInput:disabled{color:var(--dsw-alias-label-dimmed)}.RrBZSG_renameError{color:var(--dsw-alias-state-error-primary);margin-top:8px;font-size:12px;line-height:18px}.RrBZSG_deleteAction:not(:disabled){color:var(--dsw-alias-state-error-primary)}.RrBZSG_deleteStatus{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}@media (prefers-reduced-motion:reduce){.RrBZSG_wide{animation:none}.RrBZSG_search,.RrBZSG_sectionLabel,.RrBZSG_searchSlot,.RrBZSG_searchInput,.RrBZSG_headerActions{transition:none}}";
		...
		var WorkspaceBrowser_module_css_default = {
			"clearButton": "RrBZSG_clearButton",
			"deleteAction": "RrBZSG_deleteAction",
			"deleteStatus": "RrBZSG_deleteStatus",
			"empty": "RrBZSG_empty",
			"fade": "RrBZSG_fade",
			"flatList": "RrBZSG_flatList",
			"groupSection": "RrBZSG_groupSection",
			"headerActions": "RrBZSG_headerActions",
			"headerActionsHidden": "RrBZSG_headerActionsHidden",
			"iconButton": "RrBZSG_iconButton",
			"list": "RrBZSG_list",
			"listArea": "RrBZSG_listArea",
			"listTopDropActive": "RrBZSG_listTopDropActive",
			"listTopDropIndicator": "RrBZSG_listTopDropIndicator",
			"rail": "RrBZSG_rail",
			"renameError": "RrBZSG_renameError",
			"renameInput": "RrBZSG_renameInput",
			"root": "RrBZSG_root",
			"search": "RrBZSG_search",
			"searchButton": "RrBZSG_searchButton",
			"searchEmptyClear": "RrBZSG_searchEmptyClear",
			"searchExpanded": "RrBZSG_searchExpanded",
			"searchInput": "RrBZSG_searchInput",
			"searchSlot": "RrBZSG_searchSlot",
			"searchSlotExpanded": "RrBZSG_searchSlotExpanded",
			"searchStatus": "RrBZSG_searchStatus",
			"searchTree": "RrBZSG_searchTree",
			"searchWarning": "RrBZSG_searchWarning",
			"sectionHeader": "RrBZSG_sectionHeader",
			"sectionLabel": "RrBZSG_sectionLabel",
			"sectionLabelHidden": "RrBZSG_sectionLabelHidden",
			"sessionOverflowButton": "RrBZSG_sessionOverflowButton",
			"treeBody": "RrBZSG_treeBody",
			"wide": "RrBZSG_wide",
			"wide-in": "RrBZSG_wide-in",
			"workspaceDropAfter": "RrBZSG_workspaceDropAfter",
			"workspaceDropBefore": "RrBZSG_workspaceDropBefore"
		};
		//#endregion
		...
		*/
		...
		function SearchResults({ useSessions, useSessionPendingInteraction, open, workspaces, archivedSessionIds, query, remote, resultLimit, usePanelInfo, onClearSearch, t }) {
			const panelActive = usePanelInfo((info) => info.activePanelId !== null);
			const list = useSessions((s) => s);
			const pendingInteractions = useSessionPendingInteraction((s) => s);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						!pending && results.items.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: WorkspaceBrowser_module_css_default.empty,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: t("search.noMatches") }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								className: WorkspaceBrowser_module_css_default.searchEmptyClear,
								onClick: onClearSearch,
								children: [t("search.clear"), " — 返回项目列表"]
							})]
						}),
		...
		*/
		function WorkspaceBrowser({ wide, usePanelInfo, expandSidebar, useSessions, useSessionPendingInteraction, useWorkspaces, useStore, actions, startSession, open, renameSession, forkSession, renameWorkspace, deleteWorkspace, insertWorkspaceBefore, archiveSession, insertSessionBefore, createWorkspace, searchSessions, searchResultLimit, useDirectoryFlow, useHostInfo, renderSlot, t }) {
			...
			const [searchReadonly, setSearchReadonly] = (0, react.useState)(true);
