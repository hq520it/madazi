...
/**
 * madazi fork runtime marker. Comments vanish from compressed tsdown bundles,
 * so archive fuses grep this string literal to prove the deployed package came
 * from the fork build. Exported so the bundler never treeshakes it (a mere
 * constant-reference guard gets constant-folded away).
 */
export const MADAZI_TRUSTED_HOST_MARKER = 'madazi-trusted-host-bypass';
...
export class HostConnectionService extends Service {
    ...
    requestRejection(request) {
        ...
        // The `MARKER.length` guard keeps the runtime marker referenced and unshaken.
        return MADAZI_TRUSTED_HOST_MARKER.length === 0 ? 401 : undefined;
    }
    /** Authenticate an index request through the process-token exchange or cookie. */
    authorizeIndex(_request, _response) {
